...
class QuestionTableUtils {
     ...
     */
    static getTableHeader({ data, setModalState, styles, navigate, title, examId, courseId }) {
        ...
        return {
            ...
            actionButtons: [
                ...
                TableUtils.getExportButton({ url: "/questions" }),
                {
                    icon: <List />,
                    iconOnly: true,
                    onClick: () => navigate("/questions/form/add"),
                },
            ],
            filters: [
                {
    ...
    static getTableActions({ data, setModalState, setSelectedQuestion, navigate }) {
        const handleAction = (row, actionType) => {
            ...
            if (actionType === "edit") {
                navigate(`/questions/form/${row["id"]}`);
            } else {
