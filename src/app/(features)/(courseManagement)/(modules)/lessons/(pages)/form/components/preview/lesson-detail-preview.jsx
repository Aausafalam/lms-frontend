        ...
        <div className={isMobile || isTablet ? "space-y-8" : "grid grid-cols-3 gap-8"}>
          <div className={isMobile || isTablet ? "space-y-6" : "col-span-2 space-y-6"}>
            ...
            {data.instructors?.length > 0 && (
              <ContentCard
                ...
                Icon={Users}
                headerColor="blue"
                subTitle="Expert educators who will guide you through this lesson"
              >
                <div className={`grid ${isMobile ? "grid-cols-1" : "grid-cols-2"} gap-4`}>
                  {data.instructors.map((instructor, index) => (
                    ...
                    >
                      ...
                      <div className="ml-2">
                        <h3
                          className={`font-medium text-gray-900 dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors ${isMobile ? "text-xs" : "text-sm"}`}
                        >
                          {instructor.name}
                        </h3>
            ...
            {data.preRequisites?.length > 0 && data.preRequisites[0] && (
              <ContentCard
                title="Prerequisites"
                subTitle="Required knowledge and skills before starting this lesson"
                Icon={Award}
                headerColor="orange"
                isMobile={isMobile}
              >
                <div className="space-y-0">
                  {data.preRequisites
                    ...
                    .map((prerequisite, index) => (
                      <div
                        key={index}
                        className="flex items-start group hover:bg-orange-50 dark:hover:bg-orange-950/20 p-3 rounded-lg transition-colors"
                      >
            ...
            {data.resources?.length > 0 && data.resources[0]?.title && (
              <ContentCard
                ...
                subTitle="Helpful links and references to supplement your learning"
                Icon={Link2}
                headerColor="violet"
                isMobile={isMobile}
              >
                    ...
                    .map((resource, index) => (
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <div>
                            <p
                              className={`text-gray-800 dark:text-gray-200 font-bold ${isMobile ? "text-sm" : "text-base"}`}
                            >
                              {resource.title}
                            </p>
                            {resource.url && (
                              <p
                                title={resource.url}
                                className="text-gray-600 dark:text-gray-400 text-sm mt-1 truncate max-w-44 break-words"
                              >
                                {resource.url}
                              </p>
                            )}
            ...
            {data.attachments?.length > 0 && data.attachments[0]?.title && (
              <ContentCard
                ...
                subTitle="Files, documents, and materials for hands-on practice"
                Icon={Paperclip}
                headerColor="teal"
                isMobile={isMobile}
              >
                          ...
                          <div>
                            ...
                            {attachment.url && (
                              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                                {attachment.type?.toUpperCase()} Resource
                              </p>
                            )}
                          </div>
                        </div>
          ...
          {isDesktop && (
            <div className="space-y-6">
              {((data.introVideo && typeof data.introVideo === "string" && getYoutubeVideoId(data.introVideo)) ||
