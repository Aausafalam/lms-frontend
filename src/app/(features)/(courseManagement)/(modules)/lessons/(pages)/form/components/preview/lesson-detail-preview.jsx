...
/**
 * Enhanced Lesson Detail Preview Component
 * Renders a premium, responsive lesson preview with modular components
 */

export function LessonDetailPreview({ initialData, onDetailsPage, viewPort }) {
    ...
    const data = onDetailsPage ? { ...lessonDetails.data?.data, instructorIds: lessonDetails.data?.data?.instructors?.map((item) => item.id) } : initialData;
    ...
    useEffect(() => {
        onDetailsPage && lessonDetails.fetch?.({ dynamicRoute: `/${courseId}/module/${moduleId}/lesson/${lessonId}` });
        instructorList.fetch?.({ params: { responseType: "dropdown" } });
    }, [lessonId]);

    if (onDetailsPage && lessonDetails.isLoading) {
        return (
    ...
    if (onDetailsPage && lessonDetails.error) {
    ...
    if (onDetailsPage && !lessonDetails.data?.data) {
        return (
            <div className="flex items-center justify-center h-64">
    ...
    const handleEdit = () => navigate(`/lessons/form/${params.lessonDetails}?courseId=${courseId}&moduleId=${moduleId}`);
    ...
    return (
        <div className={`w-full ${(isTablet || isMobile) && !viewPort ? "" : onDetailsPage ? "max-h-[86vh] overflow-scroll" : "max-h-[75vh] overflow-scroll"} max-w-[1200px]`}>
            <Header isMobile={isMobile} data={{ ...data, number: "Lesson 1" }} badges={customBadges} onBack={handleBack} onEdit={handleEdit} onDuplicate={handleDuplicate} onDelete={handleDelete} />
            {/* Hero Section */}
                    ...
                    <div className={`${isMobile || isTablet ? "space-y-4 sm:space-y-6" : "xl:col-span-2 space-y-6"}`}>
                        ...
                        {data.learningOutcomes?.length > 0 && data.learningOutcomes[0] && (
                            <ContentCard
                                ...
                                Icon={GraduationCap}
                                headerColor="purple"
                                subTitle="Key knowledge and skills you'll gain by completing the lesson"
                                isMobile={isMobile}
                            >
                    ...
                    {(isDesktop || (isTablet && !isMobile)) && (
                        <div className="space-y-4 sm:space-y-6">
                            ...
                            <ContentCard
                                headerColor="orange"
                                title="Topic Tags"
                                subTitle="Keywords and tags for this lesson"
                                Icon={Tag}
                                className="rounded-lg shadow-sm overflow-hidden border-gray-100 bg-white dark:bg-gray-900 dark:border-gray-800"
                            >
