        ...
        <div className="space-y-4 mt-2 dark:text-white">
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                    <PremiumButton variant="outline" size="sm" className="text-[0.7rem] px-4">
                        <Download className="h-3 w-3 mr-1" /> Syllabus
                    </PremiumButton>
                    <PremiumButton size="sm" className="text-[0.7rem] px-4">
            ...
            <div className="bg-primary/5 p-3 rounded-xl border border-primary/20 flex items-center justify-between">
                ...
                <PremiumButton size="sm" className="text-[0.7rem] px-4">
                    <GraduationCap className="h-3 w-3 mr-2" /> Learning Path
                </PremiumButton>
            </div>
                            ...
                            <div className="divide-y border-t dark:border-gray-700">
                                {module.items && module.items.length > 0 ? (
                                    module.items.map((lesson, i) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-2 ml-2">
                                                {lesson.free && (
                                                    <PremiumBadge variant="outline" className="text-xs py-1">
                                                        Preview
                                                    </PremiumBadge>
                                                )}
                                                {lesson.active && (
                                                    <PremiumButton size="sm" className="bg-primary hover:bg-primary/90 rounded-full h-6 text-xs">
                                                        Continue
                                                    </PremiumButton>
                                                )}
                                                {!lesson.active && !lesson.completed && (
                                                    <PremiumBadge variant="ghost" size="icon" className="h-6 w-6 opacity-50" disabled>
                                                        <Lock className="h-3 w-3" />
                                                    </PremiumBadge>
                                                )}
                                                {lesson.completed && (
                                                    <PremiumButton variant="outline" size="sm" className="rounded-full h-6 text-xs">
                                                        Review
                                                    </PremiumButton>
                                                )}
                                            </div>
                                    ...
                                    <div className="p-4 text-center">
                                        ...
                                        <PremiumButton variant="outline" size="sm" className="mt-2 rounded-full text-xs">
                                            Prerequisites
                                        </PremiumButton>
                                    </div>
                                )}
