...
import { Play, Star, Users, Clock, Globe, Hash, CheckCircle, Flame, TrendingUp, Award, Heart, Share2 } from "lucide-react";
...
export function HeroSection({ data, isMobile, instructors, isTablet, isDesktop }) {
    ...
    return (
        <div
            className={`relative w-full bg-cover bg-center overflow-hidden ${isDesktop ? "rounded-2xl" : ""}`}
            style={{
        ...
        >
            ...
            {/* Main Content */}
            <div className="relative z-10 px-4 md:px-6 py-8 mx-auto">
                <div className={`${isDesktop ? "grid grid-cols-5 gap-8 items-center" : "space-y-6"}`}>
                    {/* Left Content */}
                    <div className={`${isDesktop ? "col-span-3" : ""} space-y-4`}>
                        {/* Compact Badges */}
                        <div className="flex flex-wrap gap-2">
                            {data.status === "published" && (
                                <Badge className="bg-emerald-500/90 hover:bg-emerald-600 text-white border-0 px-3 py-1 text-xs font-medium">
                                    <CheckCircle className="h-3 w-3 mr-1" />
                                    Live
                                </Badge>
                            )}
                            {data.isFeatured && (
                                <Badge className="bg-orange-500/90 hover:bg-orange-600 text-white border-0 px-3 py-1 text-xs font-medium">
                        ...
                        <div className="space-y-2">
                            <h1 className={`font-bold text-white leading-tight ${isMobile ? "text-xl" : isTablet ? "text-2xl" : "text-3xl"}`}>{data.name || "Course Title"}</h1>
                            <p className={`text-white/85 font-light leading-relaxed ${isMobile ? "text-sm" : "text-base"}`}>{data.summary || "Course summary will appear here"}</p>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            <div className="flex items-center bg-white/10 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/20">
                                <div className="flex items-center mr-2">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} className={`h-3 w-3 ${i < Math.floor(courseStats.rating) ? "text-orange-400 fill-orange-400" : "text-white/40"}`} />
                                    ))}
                                </div>
                            ...
                            {/* Students */}
                            <div className="flex items-center bg-white/10 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/20">
                                <Users className="h-3 w-3 mr-2 text-orange-400" />
                            ...
                            {data.code && (
                                <div className="flex items-center bg-white/10 backdrop-blur-sm rounded-full px-3 py-1.5 border border-white/20">
                                    <Hash className="h-3 w-3 mr-2 text-orange-400" />
                            ...
                            <div className="flex items-center space-x-3 bg-white/5 backdrop-blur-sm rounded-xl p-3 border border-white/10">
                                <div className="flex -space-x-2">
                                    {instructors?.slice(0, 2).map((instructor, index) => (
                                        <Avatar key={index} className="border-2 border-orange-400 h-8 w-8">
                        ...
                        <div className="flex flex-wrap gap-3 pt-2">
                            <Button className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-lg border-0 transition-all duration-300 hover:scale-105 px-6 py-2 text-sm font-semibold">
                                Enroll Now
                            </Button>

                            <Button
                                variant="outline"
                                className="bg-white/10 text-white hover:bg-orange-500/20 backdrop-blur-sm border-orange-400/50 hover:border-orange-400 transition-all duration-300 hover:scale-105 px-4 py-2 text-sm font-medium"
                            ...
                            <Button
                                variant="outline"
                                className="bg-white/10 text-white hover:bg-orange-500/20 backdrop-blur-sm border-white/30 hover:border-orange-400 transition-all duration-300 hover:scale-105 px-4 py-2 text-sm font-medium"
                            >
