...
export function SidebarNavigation({ activeSection, scrollToSection, formData, handlers }) {
  ...
  const navigationItems = [
    ...
    { id: "scheduling-access", label: "Scheduling & Access", icon: <Calendar className="h-4 w-4" /> },
    { id: "attempt-rules", label: "Attempt Rules", icon: <Clock className="h-4 w-4" /> },
    { id: "submission-rules", label: "Submission Rules", icon: <Cog className="h-4 w-4" /> },
          ...
          <nav className="space-y-1">
            {navigationItems.map((item) => (
              ...
              >
                <div
                  className={GlobalUtils.cn(
                    "mr-0 p-1 rounded-md transition-all",
                    activeSection === item.id
