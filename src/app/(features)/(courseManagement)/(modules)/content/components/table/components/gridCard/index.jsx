...
export default function ContentCard({ data }) {
    ...
    const handleCardClick = () => {
        navigate(`/content/details/${contentData.id}?courseId=${courseId}&moduleId=${moduleId}&lessonId=${lessonDetails}`);
    };

    // Format instructors names for display
    const formatInstructorNames = () => {
        if (contentData.instructors.length === 1) {
            return contentData.instructors[0].name;
        } else if (contentData.instructors.length === 2) {
            return `${contentData.instructors[0].name} & ${contentData.instructors[1].name}`;
        } else {
            return `${contentData.instructors[0].name} +${contentData.instructors.length - 1}`;
        }
    };
            ...
            <div className="relative p-4 flex flex-col flex-grow">
                ...
                <div className="mb-2 flex items-start justify-between">
                    ...
                    <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 ml-2 shrink-0">
                        ...
                        <span>{contentData.duration} hours</span>
                    </div>
                </div>

                {/* Description */}
                <p className="min-h-14 text-xs leading-relaxed text-gray-600 dark:text-gray-300 line-clamp-3" name={contentData.summary}>
                    {contentData.summary}
                </p>
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <div className="flex items-center">
                        <div className="flex -space-x-2 mr-2">
                            {contentData.instructors.slice(0, 3).map((instructors, index) => (
                                <Image
                                    key={index}
                                    width={24}
                                    height={24}
                                    ...
                                    className={GlobalUtils.cn("h-6 w-6 rounded-full ring-1 ring-white dark:ring-gray-800", contentData.featured ? "ring-orange-500/50" : "")}
                                />
                            ))}
                            {contentData.instructors.length > 3 && (
