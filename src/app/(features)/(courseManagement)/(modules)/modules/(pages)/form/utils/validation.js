/**
 * Module form validation utilities
 * @description Reusable validation functions for module forms
 */
...
export const validateArrayFields = (formData) => {
    ...
    if (!Array.isArray(formData.categoryIds) || formData.categoryIds.length === 0) {
        errors.instructorIds = "At least one category is required";
    }
...
export const validateResources = (resources) => {
    ...
    const hasIncompleteResources = resources.filter((a) => a.title?.trim() || a.link?.trim()).some((a) => !a.title?.trim() || !a.link?.trim());

    return hasIncompleteResources ? "Each resources must have title, link" : null;
};
 ...
 */
export const validateModuleForm = (formData) => {
    const errors = {
