 ...
 * @description Reusable card component for displaying module information
 */
export default function ModuleCard({ data, view, onEdit, onDelete, onView }) {
    ...
    const formatInstructorNames = () => {
        if (!moduleData.instructors?.length) return "No instructor assigned";
    ...
    const handleShare = (e) => {
        ...
        if (navigator.share) {
            navigator.share({
                ...
                text: moduleData.summary,
                url: window.location.origin + `/modules/details/${moduleData.id}?courseId=${courseId}`,
            });
        }
    };
    ...
    const getImageUrl = (type = "thumbnailUrl") => {
        ...
        return `${apiConstants.BACKEND_API_BASE_URL}/course/${courseId}/module/${moduleData.id}/getImage?type=${type}&token=${ApiUtils.getAuthToken()}`;
    };
            ...
            <div className="w-full">
                ...
                >
                    {isMobile ? (
                        // Mobile Layout
                        <>
                            {/* Module Image and Basic Info */}
                            <div className="flex items-start space-x-3">
                                <div className="relative h-16 w-20 rounded-md overflow-hidden flex-shrink-0">
                                    <Image width={80} height={64} src={getImageUrl() || "/placeholder.svg"} alt={moduleData.name} className="h-full w-full object-cover" />
                                    ...
                                    <div className="flex items-center space-x-2 mt-2">
                                        <div className="flex items-center space-x-1">
                                            {[1, 2, 3, 4, 5].map((star, index) => (
                                                <Star key={index} className={`h-3 w-3 ${index < Math.floor(moduleData.rating) ? "fill-yellow-400 text-yellow-400" : "fill-gray-300 text-gray-300"}`} />
                                            ))}
                                            <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{moduleData.rating || 0}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2" title={moduleData.summary}>
                                    {moduleData.summary || "No description provided"}
                                </p>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center space-x-2">
                                        {moduleData.tags?.slice(0, 1).map((tag, index) => (
                                            <span
                                                key={index}
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                                <div className="flex items-center space-x-4">
                                    <div className="flex items-center space-x-1">
                                        ...
                                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{moduleData.duration || 0}</span>
                        ...
                        <>
                            ...
                            <div className="col-span-4 flex items-center space-x-3">
                                <div className="relative h-12 w-16 rounded-md overflow-hidden flex-shrink-0">
                                    <Image width={64} height={48} src={getImageUrl() || "/placeholder.svg"} alt={moduleData.name} className="h-full w-full object-cover" />
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
                                    <div className="absolute top-1 right-1">
                                        <Play className="h-2 w-2 text-white" />
                                    </div>
                            ...
                            <div className="col-span-3 flex items-center space-x-4">
                                ...
                                <div className="flex items-center space-x-1">
                                    ...
                                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{moduleData.lessonCount || 0}</span>
            ...
            <div className="relative w-full overflow-hidden rounded-lg bg-white shadow-md transition-all duration-300 hover:shadow-lg dark:bg-gray-900 dark:hover:bg-gray-800">
                ...
                <div className="relative h-32 sm:h-36 w-full overflow-hidden">
                    ...
                    <div className="absolute right-2 top-2">
                        {moduleData.tags?.slice(0, 1).map((badge, index) => (
                    ...
                    <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
                        <div className="flex items-center space-x-1">
                            {[1, 2, 3, 4, 5].map((star, index) => (
                                <Star
                                    key={index}
                                    className={`h-2.5 w-2.5 sm:h-3 sm:w-3 ${index < Math.floor(moduleData.rating) ? "fill-yellow-400 text-yellow-400" : "fill-gray-400 text-gray-400"}`}
                                />
                            ))}
                            <span className="ml-1 text-xs font-medium text-white">{moduleData.rating || 0}</span>
                        </div>
                        <div className="flex items-center text-xs text-gray-200">
                            <Clock className="mr-1 h-2.5 w-2.5 sm:h-3 sm:w-3" />
                            <span>{moduleData.duration || 0}h</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative p-3 sm:p-4">
                    ...
                    {/* Description */}
                    <p className="mb-3 text-xs sm:text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{moduleData.summary || "No description available"}</p>

                    {/* Instructor */}
                    <div className="mb-3 flex items-center space-x-2">
                        <div className="flex -space-x-1">
                            {moduleData.instructors.slice(0, 2).map((instructor, index) => (
                                <Image
                                    key={index}
                            ...
                            {(moduleData.instructors?.length || 0) > 2 && (
                                <div
                                    className={`${
                                ...
                                >
                                    +{moduleData.instructors.length - 2}
                                </div>
                            )}
                        </div>
